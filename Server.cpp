...
Server::Server() : reliableListener("anyip4", 30000), syncManager(SyncManager::Server, reliableListener.GetListenerUUID())
{
	fastListener = agk::CreateUDPListener("anyip4", 30000 + 1); // Create a UDP listener for fast messages
}
...
void Server::SendFastData()
{
	...
	for (int conn = 0; conn < reliableListener.GetTotalConnections(); conn++)
	{
		RUDPListener::ConnectionUUID clientUUID = reliableListener.GetConnectionUUID(conn); // Get the UUID of the connection
		...
		for (auto& it : fastUpdateMap)
		{
			...
			if (!data.empty() && clientUUID != syncManager.GetOwningConnection(uuid))
				updateCount++;
		}
		...
		agk::SendUDPNetworkMessage(fastListener, message, reliableListener.GetConnectionIP(clientUUID), reliableListener.GetConnectionPort(clientUUID) + 1); // Send the network message to the server
	}
}

void Server::ReceiveRPCData()
{
